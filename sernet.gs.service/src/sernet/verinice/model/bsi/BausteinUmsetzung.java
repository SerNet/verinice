...
public class BausteinUmsetzung extends CnATreeElement {
    ...
    private static final Pattern KAPITEL_PATTERN = Pattern.compile("(\\d+)\\.(\\d+)"); //$NON-NLS-1$
    ...
    public int[] getKapitelValue() {
        ...
        if (m.find()) {
            try {
                ...
                Logger.getLogger(this.getClass()).error(Messages.BausteinUmsetzung_6, e);
            }
        }
        return kapitel;
    }
    ...
    /**
     * Überprüft anhand der umgesetzten Massnahmen, welche Siegestufe in diesem
     * Baustein bereits erreicht wurde.
     * 
     * @return Siegelstufe als 0, A, B, C
     */
    public char getErreichteSiegelStufe() {
        // starte mit höchster Stufe und reduziere anhand nicht-umgesetzter
        // Massnahmen:
