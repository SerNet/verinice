...
/**
 * Configuration item. Actual configuration values are saved in Entity. Can be
 * linked to a person for individual configuration items such as usernames /
...
public class Configuration implements Serializable, ITypedElement, Comparable<Configuration> {
    ...
    public String getUser() {
        return entity.getPropertyValue(PROP_USERNAME);
    }

    public String getEmail() {
        return entity.getPropertyValue(PROP_NOTIFICATION_EMAIL);
    }
    ...
    public String getPass() {
        return entity.getPropertyValue(PROP_PASSWORD);
    }
     ...
     * the content covered by a license represented by a
     * {@link LicenseManagementEntry} that is referenced via the paramater
     * licenseId.
     * 
     * To prevent confusion with duplicate database entries, (which could result
     * in unauthorized content access) in this special case, the property list
     * is checked for property existance before adding a new property
     * 
     * 
     * @param licenseId
     *            - the licenseId that should be assigned to this user
     */
    public void addLicensedContentId(String licenseId) {
        PropertyType type = getTypeFactory().getPropertyType(Configuration.TYPE_ID,
                PROP_LICENSED_CONTENT_IDS);
    ...
    /**
     * Removes one licenseId assignment from this user, given by parameter
     * licenseId
     * 
     * @param licenseId
     *            - the id to remove
    ...
    public void setNotificationEnabled(boolean b) {
        PropertyType type = getTypeFactory().getPropertyType(Configuration.TYPE_ID,
                PROP_NOTIFICATION);
        entity.setSimpleValue(type,
                (b ? "configuration_mailing_yesno_yes" : "configuration_mailing_yesno_no"));
    }
    ...
    public void setIsDeactivatedUser(boolean isDeactivated) {
        PropertyType type = getTypeFactory().getPropertyType(Configuration.TYPE_ID,
                PROP_DEACTIVATED);
        if (type == null) {
            LOG.error("Can not set property: " + PROP_DEACTIVATED
                    + ". Property type is not defined, check SNCA.xml.");
            return;
        }
        if (isDeactivated) {
    ...
    public void setNotificationExpirationEnabled(boolean b) {
        PropertyType type = getTypeFactory().getPropertyType(Configuration.TYPE_ID,
                PROP_NOTIFICATION_EXPIRATION);
        entity.setSimpleValue(type,
                (b ? "configuration_mailing_expiring_yes" : "configuration_mailing_expiring_no"));
    }

    public boolean isNotificationExpirationEnabled() {
        return isRawPropertyValueEqual(PROP_NOTIFICATION_EXPIRATION,
                "configuration_mailing_expiring_yes");
    }

    public void setNotificationExpirationDays(int days) {
        PropertyType type = getTypeFactory().getPropertyType(Configuration.TYPE_ID,
                PROP_NOTIFICATION_EXPIRATION_DAYS);
        entity.setSimpleValue(type, String.valueOf(days));
    }

    public int getNotificationExpirationDays() {
        String s = entity.getPropertyValue(PROP_NOTIFICATION_EXPIRATION_DAYS);
    ...
    public void setNotificationMeasureModification(boolean b) {
        PropertyType type = getTypeFactory().getPropertyType(Configuration.TYPE_ID,
                PROP_NOTIFICATION_MEASURE_MODIFICATION);
        entity.setSimpleValue(type, (b ? PROP_NOTIFICATION_MEASURE_MODIFICATION_YES
                : PROP_NOTIFICATION_MEASURE_MODIFICATION_NO));
    }

    public boolean isNotificationMeasureModification() {
        return isRawPropertyValueEqual(PROP_NOTIFICATION_MEASURE_MODIFICATION,
                PROP_NOTIFICATION_MEASURE_MODIFICATION_YES);
    }

    public void setAuditorNotificationExpirationEnabled(boolean b) {
        PropertyType type = getTypeFactory().getPropertyType(Configuration.TYPE_ID,
                PROP_AUDITOR_NOTIFICATION_EXPIRATION);
        entity.setSimpleValue(type, (b ? "configuration_auditmailing_expiring_yes"
                : "configuration_auditmailing_expiring_no"));
    }

    public boolean isAuditorNotificationExpirationEnabled() {
        return isRawPropertyValueEqual(PROP_AUDITOR_NOTIFICATION_EXPIRATION,
                "configuration_auditmailing_expiring_yes");
    }

    public void setAuditorNotificationExpirationDays(int days) {
        PropertyType type = getTypeFactory().getPropertyType(Configuration.TYPE_ID,
                PROP_AUDITOR_NOTIFICATION_EXPIRATION_DAYS);
        entity.setSimpleValue(type, String.valueOf(days));
    }

    public int getAuditorNotificationExpirationDays() {
        String s = entity.getPropertyValue(PROP_AUDITOR_NOTIFICATION_EXPIRATION_DAYS);
        if (s != null && s.length() > 0) {
            return Integer.parseInt(s);
        }
        // No value set, then say there is no limit.
    ...
    public void setAuditorNotificationGlobal(boolean b) {
        PropertyType type = getTypeFactory().getPropertyType(Configuration.TYPE_ID,
                PROP_AUDITOR_NOTIFICATION_GLOBAL);
        entity.setSimpleValue(type,
                (b ? PROP_AUDITOR_NOTIFICATION_GLOBAL_ALL : PROP_AUDITOR_NOTIFICATION_GLOBAL_SELF));
    }

    public void setAuditorNotificationGlobal(String value) {
        if (value != null && (!PROP_AUDITOR_NOTIFICATION_GLOBAL_ALL.equals(value))
                && (!PROP_AUDITOR_NOTIFICATION_GLOBAL_SELF.equals(value))) {
            throw new IllegalArgumentException(
                    "Value of property " + PROP_AUDITOR_NOTIFICATION_GLOBAL + " can only set to "
                            + PROP_AUDITOR_NOTIFICATION_GLOBAL_ALL + " or "
                            + PROP_AUDITOR_NOTIFICATION_GLOBAL_SELF);
        }
        PropertyType type = getTypeFactory().getPropertyType(Configuration.TYPE_ID,
                PROP_AUDITOR_NOTIFICATION_GLOBAL);
    ...
    public boolean isAuditorNotificationGlobal() {
        return isRawPropertyValueEqual(PROP_AUDITOR_NOTIFICATION_GLOBAL,
                PROP_AUDITOR_NOTIFICATION_GLOBAL_ALL);
    }

    /**
     * Returns a set of roles of the person to which this configuration belongs
     * to.
     * 
     * The roles returned herein are to be used for checking whether the user
     * has access to elements.
     * 
     * @param addUserName
     *            If true the user name is added to the result
     * @return Roles of the person to which this configuration belongs to
     */
    public Set<String> getRoles(boolean addUserName) {
        List<Property> properties = entity.getProperties(Configuration.PROP_ROLES).getProperties();
        Set<String> roles = null;
        if (properties != null) {
            roles = new HashSet<>(properties.size());
    ...
    /**
     * Returns all licenseIds the user is allowed to use. One licenseId
     * references a {@link LicenseManagementEntry} that defines the access to
     * content that is covered by special license.
     * 
     * @return Set of strings representing the ids assigned to this user
     */
    public Set<String> getAssignedLicenseIds() {
        List<Property> properties = entity.getProperties(Configuration.PROP_LICENSED_CONTENT_IDS)
                .getProperties();
    ...
    public Set<String> getStandartGroups() {
        ...
        Set<String> standartGroupsOfAccount = new HashSet<>();
     ...
     * propertytype is equal to a given value.
     * 
     * <p>
     * The method assumes that the property is single-valued.
     * </p>
     * 
     * <p>
     * The method safely handles the fact that a property might not exist at
     * all. In this case the result is always <code>false</code>
     * </p>
     * 
     * <p>
     * The given value shall not be <code>null</code>, otherwise a
     * {@link NullPointerException} is thrown.
     * </p>
     * 
     * <p>
     * The expected value is compared to the so called raw value of the
     * property. In the Hitro UI configuration this is what is provided by the
     * "id" attribute of an <em>option</em> element.
     * </p>
     * 
     * @param expected
     */
    ...
    public String toString() {
        ...
        sb.append("\nTerminwarnung: bei Ablauf in X Tagen: ")
                .append(getNotificationExpirationDays());
        sb.append("\n(Auditor) Alle Massnahmen / nur eigene: ")
                .append(isAuditorNotificationGlobal());
        sb.append("\n(Auditor) Terminwarnung: an / aus: ")
                .append(isAuditorNotificationExpirationEnabled());
        sb.append("\n(Auditor) Terminwarnung: bei Ablauf in X Tagen: ")
                .append(getAuditorNotificationExpirationDays());
        return sb.toString();
    }
