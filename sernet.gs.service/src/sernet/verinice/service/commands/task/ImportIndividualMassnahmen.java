 ...
 * exists as children of the parent baustein in the itgs catalogue
 * 
...
public class ImportIndividualMassnahmen extends GenericCommand {
    ...
    private void createIndividualMassnahmenForBausteinUmsetzung(
            BausteinUmsetzung bausteinUmsetzung) {
        if (LOG.isDebugEnabled()) {
            LOG.debug("creating " + individualMassnahmenMap.get(bausteinUmsetzung).size()
                    + " individual massnahmen for bausteinUmsetzung:\t"
                    + bausteinUmsetzung.getTitle() + "<" + bausteinUmsetzung.getUuid() + ">");
        }
        List<BausteineMassnahmenResult> bausteineMassnahmenResultList = individualMassnahmenMap
                .get(bausteinUmsetzung);
