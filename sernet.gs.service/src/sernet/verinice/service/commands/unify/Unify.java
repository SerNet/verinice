...
/**
 * Unifies a set of elements defined in a list of mappings. Each mapping
 * contains the UUID of a source and a destination element. This command copies
 * all properties from the source to the destination elements. Empty properties
 * from the source will not delete existing value in the destination. Properties
 * from the blacklist (<code>propertyTypeBlacklist</code> are ignored.
 * 
 * Optional this command also copies links from the source to the destination
 * elements.
 * 
 * This command uses the builder pattern:
 * https://en.wikipedia.org/wiki/Builder_pattern To create new instances use
 * this code:
 * 
...
public class Unify extends ChangeLoggingCommand implements IChangeLoggingCommand {
    ...
    /**
     * A list with property-ids from SNCA.xml. These properties are ignored
     * while unifying.
     */
    public static final List<String> PROPERTY_TYPE_BLACKLIST = Arrays.asList(SamtTopic.PROP_DESC,
            ...
            SamtTopic.PROP_OWNWEIGHT, SamtTopic.PROP_MIN1, SamtTopic.PROP_MIN2, Control.PROP_NAME,
            Control.PROP_DESC);
    ...
    /**
     * Creates a new Unify command with a builder. This constructor ist private.
     * To create a unify command use this code:
     * 
     * Unify unifyCommand = new Unify.Builder(mappings).copyLinks(true).build();
     * 
     * @param builder
     *            A builder for a unify command
     ...
     * 
     * If unifyinf fails for one {@link UnifyMapping} executing is continued
     * with the next element after logging the exception.
     * 
     ...
     * 
     * @param mapping
     *            A mapping with a source and an destination element
     */
     ...
     * 
     * @param source
     *            The source element of a single unify process.
     * @param destination
     *            The destination element of a single unify process.
     */
    private void unify(UnifyElement source, UnifyElement destination) throws CommandException {
        if (source == null || destination == null) {
            return;
        }
        CnATreeElement sourceElement = getDao().findByUuid(source.getUuid(),
                RetrieveInfo.getPropertyInstance());
        CnATreeElement destinationElement = getDao().findByUuid(destination.getUuid(),
                RetrieveInfo.getPropertyInstance());
        if (!dontCopyPropertyValues) {
            destinationElement.getEntity().copyEntity(sourceElement.getEntity(),
                    propertyTypeBlacklist);
        }
        if (copyLinks) {
     ...
     */
    private CnATreeElement unifyLinks(CnATreeElement sourceElement,
            CnATreeElement destinationElement) throws CommandException {
    ...
    private CnATreeElement deleteLinks(CnATreeElement element) throws CommandException {
        CnALink[] downLinks = element.getLinksDown()
                .toArray(new CnALink[element.getLinksDown().size()]);
    ...
    public CnALink createLink(CnATreeElement source, CnATreeElement destination, String relationId)
    ...
    public static class Builder {
        ...
        /**
         * @param mappings
         *            The unify mapping for the command
        ...
        /**
         * @param copyLinks
         *            If true the command copies all links from the source to
        ...
        /**
         * @param deleteSourceLinks
         *            If true the command deletes all links from the source
         *            elements.
         * @return The Unify builder
         */
        ...
        /**
         * @param dontCopyAttributes
         *            If true the command does not copy propertiy values from
         *            the source to the destination elements.
         * @return The Unify builder
         */
