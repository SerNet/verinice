...
%define xml for code examples
\usepackage{listings}
\definecolor{lightgray}{rgb}{0.97,0.97,0.97}
\lstdefinelanguage{XML}
{
  basicstyle=\ttfamily\color{gray}\bfseries\scriptsize,
...
\raisebox{-0.6\height}{\includegraphics{Icon/Control_Kataloge.png}} & Control Kataloge \\[10pt] \hline
...
\newline\\
Wenn Sie Reports für die ISM-Perspektive benötigen, lesen Sie im Unterkapitel {\em \ref{Reports erstellen} \nameref{Reports erstellen}} des Kapitels
...
\item \textbf{Zeichenkodierung (Encoding) neuer Control Kataloge:}
  ...
  (siehe Abschnitt {\em \ref{Generischer CSV-Import}
    \nameref{Generischer CSV-Import}}). Es wird empfohlen, die
...
nachfolgenden Abschnitten dargestellt. Die einzelnen Vorgehensweisen werden im Abschnitt {\em \ref{Import aus dem GSTOOL} \nameref{Import aus dem GSTOOL}} beschrieben.
...
Die Einstellungen zur Sprachumschaltung finden Sie nicht im Einstellungsmenü der Applikation. Die Umstellung der Sprache wird an einer anderen
Stelle konfiguriert. Lesen Sie in den nun folgenden Unterabschnitten, wie Sie eine Sprachumschaltung bewerkstelligen können.

\subsection{Im Standalone-Modus}
verinice verwendet standardmäßig die Sprache des Betriebssystems. Ein manuelles Festlegen der Sprache ist jedoch möglich.
\newline\\
Wenn verinice im \textbf{Standalone-Modus} verwendet wird, muss die Konfiguration nur an einer Stelle angepasst werden: in der
Konfigurationsdatei \newline \textbf{\textless VERINICE\_HOME\textgreater/verinice.ini}:
\newline\\
\colorbox{lightgray}{\parbox{\textwidth}{
{\tt -Dosgi.nl=en}
}}
\newline\\
Wenn verinice wieder in deutscher Sprache benutzt werden soll, muss Folgendes eingetragen sein:
\newline\\
\colorbox{lightgray}{\parbox{\textwidth}{
{\tt -Dosgi.nl=de}
}}
\newline\\
Die Applikation muss beendet und neu gestartet werden, bevor diese Änderungen aktiv werden.

\subsection{Im Servermodus}
Auf dem verinice.\textsc{PRO} Server kann als Sprache {\em Englisch} oder {\em Deutsch} eingestellt werden. Dazu ist die Datei
\textbf{/usr/share/tomcat6/conf/tomcat6.conf} zu bearbeiten. Lassen Sie diese Änderung durch ihren Systemadministrator vornehmen.
\newline\\
Stellen Sie folgendes ein, um die englische Sprache zu erhalten:
\newline\\
\colorbox{lightgray}{\parbox{\textwidth}
{\tt JAVA\_OPTS=\dq-Xms512M -Xmx16G -Duser.language=en -Duser.region=EN\dq}}
\newline\\
Stellen Sie folgendes ein, um die deutsche Sprache zu erhalten:
\newline\\
\colorbox{lightgray}{\parbox{\textwidth}
{\tt JAVA\_OPTS=\dq-Xms512M -Xmx16G -Duser.language=de -Duser.region=DE\dq}}
\newline\\
Unabhängig davon kann jeder Client individuell auf Deutsch oder Englisch umgestellt werden.
Die Benutzeroberfläche auf dem Client wird in der lokal eingestellten Sprache angezeigt, die Einstellung auf
dem Server hat z.B. Auswirkungen darauf, in welcher Sprache neue Datenbankobjekte angelegt werden. (z.B. Mitarbeiter mit
dem Titel ``Neue Person'' oder ``New Person'').
\newline\\
Für den praktischen Betrieb empfiehlt es sich, vorab eine Sprache für die einzugebenden Daten vorzugeben und diese auch auf
dem Server verbindlich einzustellen.
...
\textbf{Organisation/Scope \includegraphics[height=2ex]{Icon/GS_Modell.png} \textgreater Personen} \includegraphics[height=2ex]{Icon/Mitarbeiter.png}. Das Anlegen von Objekten wird in den Kapiteln {\em \ref{Strukturanalyse} \nameref{Strukturanalyse}} und
...
in der Werkzeugleiste auf den Reportablage-View-Button \includegraphics[height=2ex]{Icon/folder_table.png}.
...
\subsection[Reports löschen]{\begin{minipage}[t]{0.4\textwidth}
	...
	\end{minipage}
	\begin{minipage}{0.6\textwidth}
	  \includegraphics[height=2ex]{Icon/Delete.png}
	\end{minipage}
...
\subsection[View aktualisieren]{\begin{minipage}[t]{0.4\textwidth}
	...
	\end{minipage}
	\begin{minipage}{0.6\textwidth}
	  \includegraphics[height=2ex]{Icon/Aktualisieren.png}
	\end{minipage}
	}
