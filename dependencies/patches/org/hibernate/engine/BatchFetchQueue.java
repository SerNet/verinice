//$Id: BatchFetchQueue.java 10022 2006-06-15 19:41:25Z steve.ebersole@jboss.com $
...
public class BatchFetchQueue {
	...
	public Serializable[] getCollectionBatch(
		...
		// iterate over entries inside IdentityMap with callback 
		((IdentityMap)context.getCollectionEntries()).iterate(new IdentityMap.CallbackIterator() {
			...
			public boolean iterate(PersistentCollection collection, CollectionEntry ce) {
				if ( !collection.wasInitialized() && ce.getLoadedPersister() == collectionPersister ) {

					if ( checkForEnd && i == end ) {
						return true; //the first key found after the given key
					}
