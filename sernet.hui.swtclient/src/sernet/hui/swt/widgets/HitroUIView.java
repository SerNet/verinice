 ...
 * <code>DynamicDocumentation</code> instance. The user can select or enter
 * values, which are saved to and retrieved from a relational database.
 * 
 * 
...
public class HitroUIView implements IEntityChangedListener {
     ...
     *            the top scrolled composite, height will be adapted to created
     *            fields
     * @param formComposite
     *            parent container for dynamic - and possibly additional -
     *            widgets
     * @param huiComposite
    ...
    public void setInputHelper(String typeID, final IInputHelper helper, int type, final boolean showHint) {
        ...
        ContentProposalAdapter adapter = new ContentProposalAdapter(control, new TextContentAdapter(), new SimpleContentProposalProvider(helper.getSuggestions()), keyStroke, null);
    ...
    protected void showPopupList(Control control, IInputHelper helper) {
        ...
        Rectangle rect = control.getBounds();
        Point pt = control.getParent().toDisplay(rect.x, rect.y);
        String choice = popup.open(new Rectangle(pt.x, pt.y, control.getBounds().width, control.getBounds().height));
     ...
     * @param useRules
     *            use validation and default-value rules?
     * @param tags
     *            use tags to filter view?
     * @param taggedPropertiesOnly
     *            true: show only properties with matching tag (filter out all
     *            without tag or with different tags) false: show properties
     *            without tags and those with matching tag (filter out all with
     *            different tag)
     * @throws DBException
     */
    ...
    private void createField(PropertyType type, Composite parent) {
        ...
        if (type.isURL()) {
            ...
            createTextField(type, editableField, parent, type.isFocus(), 1 /*
                                                                            * one
                                                                            * line
                                                                            * high
                                                                            */);
            ...
            createTextField(type, editableField, parent, type.isFocus(), TEXTBOX_NUM_LINES /*
                                                                                            * three
                                                                                            * lines
                                                                                            * high
                                                                                            */);
     ...
     * @param focus
     */
    private void createNumericSelect(PropertyType fieldType, boolean editableField, Composite parent, boolean focus) {
        NumericSelectionControl sglControl = new NumericSelectionControl(entity, fieldType, parent, editableField);
        sglControl.create();
        if (focus) {
    ...
    private void createBooleanSelect(PropertyType fieldType, boolean editableField, Composite parent, boolean focus) {
        BooleanSelectionControl sglControl = new BooleanSelectionControl(entity, fieldType, parent, editable);
        sglControl.create();
        if (focus) {
            focusField = sglControl;
        }
    ...
    private void createURLField(PropertyType type, boolean editableField, Composite parent) {
    ...
    private int calculateMinHeight(Composite comp) {
        ...
        // add 2 for margin (1px at top, 1px at bottom, see contentCompLayout in
        // HitroUIComposite.java)
        return size + 2;
    }

    private void createTextField(PropertyType type, boolean editable, Composite parent, boolean focus, int lines) {
     ...
     */
    private void createMultiOptionField(PropertyType type, boolean editable, Composite parent, boolean focus, boolean reference, boolean crudButtons) {
        MultiSelectionControl mlControl = new MultiSelectionControl(entity, type, parent, editable, reference, crudButtons);
        mlControl.create();
        if (focus) {
    ...
    private void createSingleOptionField(PropertyType fieldType, boolean editable, Composite parent, boolean focus) {
        SingleSelectionControl sglControl = new SingleSelectionControl(entity, fieldType, parent, editable);
        sglControl.create();
        if (focus) {
            focusField = sglControl;
        }
    ...
    private void createDateField(PropertyType fieldType, boolean editable, Composite parent, boolean focus) {
        DateSelectionControl dateCtl = new DateSelectionControl(entity, fieldType, parent, editable, this.useRules);
        dateCtl.create();
        if (focus) {
            focusField = dateCtl;
        }
     ...
     * 
     * @see
     * sernet.snkdb.connect.docproperties.IDynDocChangedListener#dependencyChanged
     * (sernet.snkdb.guiswt.multiselectionlist.MLPropertyType,
     ...
     * 
     * @see
     * sernet.snkdb.connect.docproperties.IDynDocChangedListener#selectionChanged
     * (sernet.snkdb.guiswt.multiselectionlist.MLPropertyType,
     * sernet.snkdb.guiswt.multiselectionlist.MLPropertyOption)
     */
