...
public class HitroUIView implements IEntityChangedListener {
     ...
     * @param tabFolderMain
     */
    public HitroUIView(ScrolledComposite scrolledComposite, Composite formComposite,
            Composite huiComposite) {
     ...
     *            different tag)
     * @param validationMap
     *            contains validation results for {@link PropertyType}s
     * @throws DBException
     */
    ...
    private void createField(PropertyType type, Composite parent, boolean showValidationHint,
        ...
        if (overrides.containsKey(type.getId())) {
            ...
            createTextField(type, editableField, parent, type.isFocus(),
                    1 /* one line high */, showValidationHint, useValidationGuiHints);
            ...
            createMultiOptionField(type, editableField, parent, type.isFocus(), true,
                    type.isCrudButtons(), false, showValidationHint, useValidationGuiHints);
        } else if (type.isMultiselect()) {
            createMultiOptionField(type, editableField, parent, type.isFocus(), false, false, false,
            ...
            createMultiOptionField(type, editableField, parent, type.isFocus(), false,
                    type.isCrudButtons(), true, showValidationHint, useValidationGuiHints);
        }
    }
    ...
    private boolean tagMatches(String propertyTags) {
        if (filterTags == null || filterTags.length == 0 || propertyTags == null
    ...
    private void createNumericSelect(PropertyType fieldType, boolean editableField,
            ...
            boolean useValidationGuiHints) {
        NumericSelectionControl sglControl = new NumericSelectionControl(entity, fieldType, parent,
                editableField, showValidationHint, useValidationGuiHints);
        sglControl.create();
        if (focus) {
    ...
    private void createBooleanSelect(PropertyType fieldType, Composite parent, boolean focus) {
        BooleanSelectionControl sglControl = new BooleanSelectionControl(entity, fieldType, parent,
                editable);
        sglControl.create();
        if (focus) {
            focusField = sglControl;
    ...
    private void createURLField(PropertyType type, Composite parent, boolean showValidationHint,
            boolean useValidationGuiHints) {
    ...
    private void createMultiOptionField(PropertyType type, boolean editable, Composite parent,
            boolean focus, boolean reference, boolean crudButtons, boolean cnalinkreference,
            boolean showValidationHint, boolean useValidationGuiHints) {
        MultiSelectionControl mlControl = new MultiSelectionControl(entity, type, parent, editable,
                reference, crudButtons, cnalinkreference, showValidationHint,
                useValidationGuiHints);
        mlControl.create();
        if (focus) {
    ...
    private void createSingleOptionField(PropertyType fieldType, boolean editable, Composite parent,
            boolean focus, boolean showValidationHint, boolean useValidationGuiHints) {
        SingleSelectionControl sglControl = new SingleSelectionControl(entity, fieldType, parent,
                editable, showValidationHint, useValidationGuiHints);
        sglControl.create();
        if (focus)
            focusField = sglControl;
        fields.put(fieldType.getId(), sglControl);
    ...
    private void createDateField(PropertyType fieldType, boolean editable, Composite parent,
            boolean focus, boolean showValidationHint, boolean useValidationGuiHints) {
        DateSelectionControl dateCtl = new DateSelectionControl(entity, fieldType, parent, editable,
                this.useRules, showValidationHint, useValidationGuiHints);
        dateCtl.create();
        if (focus) {
            focusField = dateCtl;
        }
