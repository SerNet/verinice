 ...
 ******************************************************************************/
package sernet.verinice.oda.driver.designer.impl;
...
public class CustomDataSetWizardPage extends DataSetWizardPage {
	...
	private transient Text m_setupQueryTextField;
	...
	private Control createPageControl(Composite parent) {
		...
		fieldLabel.setText("&BSH setup query Text:");

		m_setupQueryTextField = new Text(composite, SWT.BORDER | SWT.V_SCROLL
				| SWT.H_SCROLL);
		GridData data = new GridData(GridData.FILL_BOTH);
		data.heightHint = 100;
		m_setupQueryTextField.setLayoutData(data);
		m_setupQueryTextField.addModifyListener(new ModifyListener() {
	...
	private void initializeControl() {
		...
		String setupQueryText = ((dataSetDesign.getPrivateProperties() != null) ? dataSetDesign
				.getPrivateProperties()
				.getProperty(Query.PROP_SETUP_QUERY_TEXT)
				: null);

		if (queryText == null && setupQueryText == null)
			return; // nothing to initialize

		// initialize control
		m_queryTextField.setText((queryText == null ? "" : queryText));
		m_setupQueryTextField.setText((setupQueryText == null ? "" : setupQueryText));
		validateData();
		setMessage(DEFAULT_MESSAGE);
	}
	...
	private String getSetupQueryText() {
		// Setup query text may be non-existent.
		String text = m_setupQueryTextField.getText();
	...
	private void validateData() {
		boolean isValid = (m_queryTextField != null && getQueryText() != null && getQueryText()
				.trim().length() > 0);
	...
	private void savePage(DataSetDesign dataSetDesign) {
		...
		props.put(Query.PROP_SETUP_QUERY_TEXT, getSetupQueryText());
		
		dataSetDesign.setQueryText(queryText);
		dataSetDesign.setPrivateProperties(DesignUtil.convertToDesignProperties(props));
