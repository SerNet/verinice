...
public class LicenseManagementTier3Test extends CommandServiceProvider {
    ...
    public void deEncrypt() {
        ...
        String decryptedEntryContentId = licenseManagementService.decrypt(entry,
    ...
    public void licenseEntriesForUserByContentId() {
        try {
            ...
            int count = 0;
            for (LicenseManagementEntry entry : licenseManagementService.getExistingLicenses()) {
                String plainContentId = licenseManagementService.decrypt(entry,
                        LicenseManagementEntry.COLUMN_CONTENTID);
                if (CONTENT_ID.equals(plainContentId)) {
                    String plainLicenseId = licenseManagementService.decrypt(entry,
            ...
            LOG.error("Something went wrong with assigning a licenseId to " + "user:\t"
                    + TEST_USERNAME);
        }
    }
    ...
    private File addLicenseToRepository(LicenseManagementEntry entry)
            throws LicenseManagementException, IOException {
        File vnlFile = null;
        try {
            vnlFile = writeLMEntryToTempFile(entry, TEMP_FILE_NAME, TEMP_FILE_NAME_EXT);
    ...
    public void licensingInfos() {
        try {
            ...
            String licenseIdExpired = licenseManagementService.decrypt(expiredEntry,
                    LicenseManagementEntry.COLUMN_LICENSEID);
            String plainContentIdExpired = licenseManagementService.decrypt(expiredEntry,
                    LicenseManagementEntry.COLUMN_CONTENTID);

            String licenseIdValid = licenseManagementService.decrypt(validEntry,
                    LicenseManagementEntry.COLUMN_LICENSEID);
            String plainContentIdValid = licenseManagementService.decrypt(validEntry,
                    LicenseManagementEntry.COLUMN_CONTENTID);

            String encryptedContentIdExpired = getDynamicEncryptedString(plainContentIdExpired,
                    getUserPassword(expiredEntry), decodeEntryProperty(expiredEntry.getSalt()));
            String encryptedContentIdValid = getDynamicEncryptedString(plainContentIdExpired,
                    getUserPassword(validEntry), decodeEntryProperty(validEntry.getSalt()));
            ...
            LicenseMessageInfos infosExpired = licenseManagementService
                    .getLicenseMessageInfos(TEST_USERNAME, encryptedContentIdExpired, "", null);
            LicenseMessageInfos infosValid = licenseManagementService
    ...
    private String getDynamicEncryptedString(String plainValue, char[] password, String salt) {
    ...
    public void testIfUserIsValidForLicenseId() {
        ...
        try {
            ...
            for (LicenseManagementEntry entry : licenseEntries) {
                String licenseId = licenseManagementService.decrypt(entry,
                        LicenseManagementEntry.COLUMN_LICENSEID);
                licenseManagementService.addLicenseIdAuthorisation(getConfiguration(TEST_USERNAME),
                        licenseId);

                LocalDate validUntil = licenseManagementService.decrypt(entry,
                        LicenseManagementEntry.COLUMN_VALIDUNTIL);
                if (validUntil.isAfter(LocalDate.now())) {
                    Assert.assertTrue(licenseManagementService.isCurrentUserValidForLicense(
                            TEST_USERNAME, entry.getLicenseID(), false));
                    Assert.assertTrue(
                            "user " + TEST_USERNAME
                                    + " is no longer authorised to use content from :\t"
                                    + entry.getContentIdentifier(),
                            licenseManagementService.isUserAssignedLicenseStillValid(TEST_USERNAME,
                                    entry.getLicenseID(), false));
                } else {
                    Assert.assertFalse(licenseManagementService.isCurrentUserValidForLicense(
                            TEST_USERNAME, entry.getLicenseID(), false));
                    Assert.assertFalse(
                            "user " + TEST_USERNAME
                                    + " is no longer authorised to use content from :\t"
    ...
    public void getEntryFromRepository() throws IOException {
        ...
        try {
            ...
            String plainContentId = licenseManagementService.decrypt(entries.iterator().next(),
                    LicenseManagementEntry.COLUMN_CONTENTID);
    ...
    public void testRemovingUsersFromLicenseId()
        ...
        // ensure both entries do have the same contentid since this is what we
        // need for the test
        ...
        try {
            ...
            Assert.assertTrue("more users assigned than allowed by licenseEntry",
                    licenseManagementService
                            .hasLicenseIdAssignableSlots(firstEntry.getLicenseID()));
            Assert.assertTrue("more users assigned than allowed by licenseEntry",
                    licenseManagementService
                            .hasLicenseIdAssignableSlots(secondEntry.getLicenseID()));

            int contentIDCount = licenseManagementService
                    .getContentIdAllocationCount(secondEntry.getContentIdentifier());
            ...
            Assert.assertTrue("removing user assignments for licenseId " + firstEntry.getLicenseID()
                    + " failed", countFor1 < contentIDCount);
    ...
    private de.sernet.model.licensemanagement.LicenseManagementEntry marshalEntryToXMLObject(
    ...
    public void manualMarshallingTest() throws IOException {
        ...
        File vnlFile1 = writeLMEntryToTempFile(entry, TEMP_FILE_NAME + "EINS", TEMP_FILE_NAME_EXT);
        ...
        try {
            ...
            Assert.assertTrue(objectFromHD.getE1().equals(entry.getContentIdentifier()));
    ...
    private Organization createTestOrganization() throws CommandException {
        ...
        paramter.setLogin(TEST_USERNAME).setIsAdmin(false).setIsScopeOnly(false)
                .setFamilyName("Duck").setFirstName("Dagobert");
