...
public final class AccountSearchQueryFactory {
    ...
    private static void createWhere(StringBuilder sbHql, IAccountSearchParameter parameter) {
        ...
        for (int i = 0; i < parameter.getNumberOfAccountParameter(); i++) {
            ...
            if (((i + 1) < parameter.getNumberOfAccountParameter())
                    || parameter.isPersonParameter()) {
                sbHql.append(" and");
            }
        }
        for (int i = 0; i < parameter.getNumberOfPersonParameter(); i++) {
    ...
    private static List<Object> addParameter(IAccountSearchParameter parameter) {
        ...
        if (parameter.isAdmin() != null) {
            ...
            parameterList.add(parameter.isAdmin() ? Configuration.PROP_ISADMIN_YES
                    : Configuration.PROP_ISADMIN_NO);
        }
        if (parameter.isLocalAdmin() != null) {
            parameterList.add(Configuration.PROP_ISLOCALADMIN);
            parameterList.add(parameter.isLocalAdmin() ? Configuration.PROP_ISLOCALADMIN_YES
                    : Configuration.PROP_ISLOCALADMIN_NO);
        }
        if (parameter.isScopeOnly() != null) {
            parameterList.add(Configuration.PROP_SCOPE);
            parameterList.add(parameter.isScopeOnly() ? Configuration.PROP_SCOPE_YES
                    : Configuration.PROP_SCOPE_NO);
        }
    ...
    private static String createAccountPropertyJoin(int i) {
        ...
        sb.append(" inner join fetch entity_").append(i)
                .append(".typedPropertyLists as propertyList_").append(i);
        sb.append(" inner join fetch propertyList_").append(i).append(".properties as props_")
                .append(i);
        return sb.toString();
    }
    ...
    private static String createPersonPropertyJoin(int i) {
        ...
        sb.append(" inner join fetch pEntity_").append(i)
                .append(".typedPropertyLists as pPropertyList_").append(i);
        sb.append(" inner join fetch pPropertyList_").append(i).append(".properties as pProps_")
                .append(i);
        return sb.toString();
    }
