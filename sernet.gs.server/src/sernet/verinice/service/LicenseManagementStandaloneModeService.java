...
/**
 * 
 * This services re-defines the methods that are dealing with the
 * user-management regarding the license-management. This is necessary because
 * of only 1 existing "user" in standalone- mode. This user does not need to get
 * assignment of licenses, he is allowed to use every license that exists within
 * the workspace
 * 
 * 
 * All methods not connected to user-specific details are defined in
 * {@link LicenseManagementServerModeService} which this class is inheriting
 * from
 * 
...
public class LicenseManagementStandaloneModeService extends LicenseManagementServerModeService
     ...
     * and ignores the username, since in standalone mode username does not
     * bother
     * 
     * @param encryptedLicenseId
     *            - encrypted licenseId (not contentId!) to validate time for
     */
    @Override
    public boolean isUserAssignedLicenseStillValid(String user, String encryptedLicenseId,
        ...
        for (LicenseManagementEntry entry : getExistingLicenses()) {
            if (entry.getLicenseID().equals(encryptedLicenseId)) {
                long validUntil = decrypt(entry, LicenseManagementEntry.COLUMN_VALIDUNTIL);
     ...
     * him the usage of the content referenced by (encrypted )licenseId in
     * standalone, the existance of an {@link LicenseManagementEntry} (and a
     * validUser count > 0) allows the (singlemode) user to use the content, so
     * we do not have to check for any free slots, since there is only 1 slot we
     * can use. so just check for slotsize > 0
     * 
     * @param encryptedLicenseId
     *            - the encrypted licenseId (not contentId!) to validate
     * 
     */
    @Override
     ...
     * (referenced by licenseId) in server-mode. In standalone mode, the
     * existance of a {@link LicenseManagementEntry} for a contentId allows the
     * user to use a license, no need for assignments here
     * 
    ...
    /**
     * Get location of vnl-repository, in tier2 it is <$workspace>/vnl (not
     * configurable)
     */
    @Override
    public File getVNLRepository() throws LicenseManagementException {
        File location = null;
        try {
            String instanceAreaVNL = FilenameUtils
                    .concat(System.getProperty(IVeriniceConstants.OSGI_INSTANCE_AREA), "vnl");
     ...
     * @throws LicenseManagementException
     */
    private Set<LicenseManagementEntry> mapVNLFilesToObjects(Set<String> vnlFiles)
            throws LicenseManagementException {
    ...
    /**
     * Returns a set of file-paths that are having the extension ".vnl" and are
    ...
    /**
     * creates a file/directory at the given location (should be a directoy,
     * file makes no sense here)
     * 
     * @param location
     * @throws LicenseManagementException
     */
    private void createVNLLocation(File location) throws LicenseManagementException {
        try {
            Files.createDirectory(Paths.get(location.toURI()));
        } catch (IOException e) {
            StringBuilder sb = new StringBuilder();
            sb.append("Error creating ").append(location.getAbsolutePath())
    ...
    /**
     * creates instance of {@link LicenseMessageInfos} that wraps information
     * about a {@link LicenseManagementEntry}
     */
    @Override
    public LicenseMessageInfos getLicenseMessageInfos(String user, String encryptedContentId,
        ...
        if (firstEntry != null) {
            String contentId = decrypt(firstEntry, LicenseManagementEntry.COLUMN_CONTENTID);
            String licenseId = decrypt(firstEntry, LicenseManagementEntry.COLUMN_LICENSEID);
            LocalDate validUntil = decrypt(firstEntry, LicenseManagementEntry.COLUMN_VALIDUNTIL);
    ...
    /**
     * returns a license for the (given) default user, if existant considering
     * the given contentId
     * 
     * @param user
     ...
     * @throws LicenseManagementException
     */
    private LicenseManagementEntry getLicenseEntryToUseForDecryption(String encryptedContentId)
            throws LicenseManagementException {
        for (LicenseManagementEntry existingEntry : getExistingLicenses()) {
            try {
