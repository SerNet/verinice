 ...
 * 
 * GsmAssetScenarioRemover removes links between assets and scenarios. Method
 * deleteAssetScenarioLinks is called after process activity
...
public class GsmAssetScenarioRemover {
    ...
    /**
     * Deletes all links between assets and scenarios for one process. Method is
     * called when a task is finished.
     * 
     * After deleting the links. State of controls is updated. If scenarios a
     * has no more links to assets control state is implemented: yes. If there
     * are any links left state is implemented: partly.
     * 
    ...
    class DeleteAssetScenarioLinks implements HibernateCallback {
        ...
        private int handleScenario(CnATreeElement scenario) {
            ...
            Set<CnATreeElement> processLinkedAssets = createIntersection(allLinkedAssets,
                    processAssets);
        ...
        private int deleteAssetScenarioLinks(Query query, CnATreeElement scenario,
                Set<CnATreeElement> assets) {
         ...
         * it (via a scenario).
         * 
         * If all linked assets are part of this process, state is
         * IControl.IMPLEMENTED_YES, otherwise is IControl.IMPLEMENTED_PARTLY.
         * 
         * @param processLinkedAssets
         *            Number of assets linked to one control (via a scenario)
         *            which are part of this process
         * @param allLinkedAssets
         *            Number of assets linked to one control via a scenario
         * @return Implementation state of an control:
         *         IControl.IMPLEMENTED_PARTLY or IControl.IMPLEMENTED_YES
         */
        private String determineState(int numberOfProcessLinkedAssets,
                int numberOfAllLinkedAssets) {
            String state = IControl.IMPLEMENTED_PARTLY;
            if (numberOfAllLinkedAssets == 0
                    || (numberOfAllLinkedAssets == numberOfProcessLinkedAssets)) {
        ...
        private Set<CnATreeElement> createIntersection(Set<CnATreeElement> elementSet,
                List<CnATreeElement> elementList2) {
