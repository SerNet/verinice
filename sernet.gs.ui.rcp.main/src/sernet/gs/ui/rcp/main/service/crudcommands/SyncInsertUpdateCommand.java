...
{	
	...
	public SyncInsertUpdateCommand( String sourceId, Element syncDataElement, Element syncMappingElement, 
			boolean insert, boolean update, List<String> errorList )
	{	
	 ...
	 * execute()
	 * 
	 * Processes the given <syncData> and <syncMapping> elements
	 * in order to insert and/or update objects in(to) the database,
	 * according to the flags insert & update.
	 * 
	 * If there already exists an ITVerbund from a past sync session,
	 * this one (identified by its sourceID) will be used; otherwise
	 * this creates a new one within the BSIModel. 
	 * 
	...
	public void execute()
	{
		...
		try
		{
			cmdLoadBSIModel = ServiceFactory.lookupCommandService().executeCommand( cmdLoadBSIModel );
		}
		...
		if ( null == importITVerbund )
			try{ importITVerbund = createNewItVerbund( model, sourceId ); }
			catch ( Exception e1 ){throw new RuntimeCommandException( "Fehler beim Anlegen eines ITVerbundes" );}
		
		Iterator iterator = syncDataElement.getChildren( "syncObject", SyncNamespaceUtil.DATA_NS ).iterator();
		...
		while( iterator.hasNext() )
		{
			...
			// the element node with (object type)extId = obj.extObjectType from syncMapping:

			Element mapObj = null;
			
			try
			{
				// XPath Expression: //map:mapObjectType[@map:extId="extObjectType"]
				String xPathExpression = "//map:mapObjectType[@extId=\""+extObjectType + "\"]";
				//String xPathExpression = "//map:mapObjectType[@externalId=\""+extObjectType + "\"]";
			...
			{
				errorList.add( "Konnte XPath-Ausdruck zur Selektion der Objekttypen nicht anwenden!" );
				return;
			}
			...
			if( elementInDB != null )	// object found!
			{
				...
				else			// do not update this object's attributes!
					setAttributes = false;
			}
			
			if( null == elementInDB && insert ) // nothing found -> create new object, if "insert" flag is set:
			{
				/*** INSERT: ***/
				...
				try
				{
					...
					CreateElement<CnATreeElement> createElement = new CreateElement<CnATreeElement>( container, getClassFromTypeId( veriniceObjectType ) ) ;
					ServiceFactory.lookupCommandService().executeCommand( createElement );
			...
			/* Now if we should update an existing object or
			 * created a new object, set the associated attributes:		*/
			if( null != elementInDB && setAttributes )
			{
				// for all <syncAttribute>-Elements below current <syncObject>...
				Iterator attrIterator = obj.getChildren( "syncAttribute", SyncNamespaceUtil.DATA_NS ).iterator();
				...
				while( attrIterator.hasNext() )
				{
					...
					{
						// XPath Expression: //mapObjectType[@extId="extObjectType"]/mapAttributeType[@extId="attrExtId"]
						String xPathExpression = "//map:mapObjectType[@extId=\""+extObjectType + "\"]/map:mapAttributeType[@extId=\"" + attrExtId +"\"]";
						//String xPathExpression = "//map:mapObjectType[@externalId=\""+extObjectType + "\"]/map:mapAttributeType[@extId=\"" + attrExtId +"\"]";
						XPath xPath = XPath.newInstance( xPathExpression );
						xPath.addNamespace( SyncNamespaceUtil.DATA_NS );
						xPath.addNamespace( SyncNamespaceUtil.MAPPING_NS );
						xPath.addNamespace( SyncNamespaceUtil.SYNC_NS );
						mapAttr = (Element)xPath.selectSingleNode( this.syncMappingElement );
					}
					catch ( JDOMException e )
					{
						this.errorList.add( "Konnte XPath-Ausdruck zur Selektion der Attribute nicht anwenden!" );
					}
					...
					{					
						...
						PropertyType propertyType = elementInDB.getEntityType().getPropertyType( attrIntId );
	 ...
	 * findDbElement()
	 * 
	 * Query element (by externalID) from DB, which has been
	 * previously synchronized from the given sourceID.
	 * 
	 * @param sourceID
	 * @param externalID
	 * @return the CnATreeElement from the query
	...
	private CnATreeElement findContainerFor( ITVerbund verbund, String veriniceObjectType )
	{
		...
		// TODO: passt das so?
		// teilweise gab's Probleme mit dem Objekt verbund.getCategory(), welches
		// z.B. beim Einfügen in eine vorher bereits einmal "benutzte" Kategorie nur
		// ein HibernateProxy war und nicht das "echte" Objekt?!
		if( container instanceof HibernateProxy )
			return (CnATreeElement)( (HibernateProxy)container ).getHibernateLazyInitializer().getImplementation();
		else
			return container;
	}
	...
	private ITVerbund createNewItVerbund( BSIModel model, String sourceId ) throws Exception
	{
		...
		CreateImportITVerbund command = new CreateImportITVerbund( model, ITVerbund.class, sourceId );
		command = ServiceFactory.lookupCommandService().executeCommand( command );
