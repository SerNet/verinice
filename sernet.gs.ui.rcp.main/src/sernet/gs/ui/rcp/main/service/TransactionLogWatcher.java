...
public class TransactionLogWatcher {
    ...
    private void process(ChangeLogEntry changeLogEntry, CnATreeElement changedElement) {
        ...
        if (log.isInfoEnabled()) {
            log
                    .info("Processing change event type " + changeLogEntry.getChangeDescription()
                            + " from user " + changeLogEntry.getUsername() + " for element "
                            + changeLogEntry.getElementClass() + " / "
        ...
        case ChangeLogEntry.TYPE_INSERT:
            if (changedElement != null) {
                ...
                CnAElementFactory
                        .getModel(changedElement)
                        .childAdded(changedElement.getParent(), changedElement);
            }
            break;
        ...
        default:
            ...
            Logger
                    ...
                    .debug("Unrecognized change type received from server. Ignored, type ID was: "
                            + changetype);
            break;
        }
    }
