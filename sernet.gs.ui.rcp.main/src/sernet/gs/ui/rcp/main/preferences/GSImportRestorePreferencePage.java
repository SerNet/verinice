...
public class GSImportRestorePreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	public void createFieldEditors() {
		...
		gstoolDumpFile = new FileFieldEditor(PreferenceConstants.GSTOOL_RESTOREDB_FILE, "Datei mit Sicherung aus GSTOOL", getFieldEditorParent()) {
			@Override
		...
		attachDb = new StringFieldEditor(PreferenceConstants.GS_DB_RESTOREDB_NAME, "Datenbank wiederherstellen als Name", getFieldEditorParent());
		addField(attachDb);

		toDirField = new DirectoryFieldEditor(PreferenceConstants.GS_DB_RESTOREDB_TODIR, "Datenbank wiederherstellen in Verzeichnis", getFieldEditorParent()) {
			@Override
			public boolean isValid() {
	...
	private void createAttachButton() {
		...
		button.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				boolean doIt = MessageDialog.openConfirm(getShell(), "Datenbank wiederherstellen", "Verbindung wird hergestellt mit " + urlString + "\n\nDie Datei '" + fileName + "' wird wiederhergestellt als Datenbank '" + newDbName + "' im Verzeichnis '" + toDir + "'. Einverstanden?");
				if (!doIt) {
					return;
				}
						...
						public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
							...
							try {
								...
								Display.getDefault().syncExec(new Runnable() {
									public void run() {
										MessageDialog.openInformation(getShell(), "Hurra", "Die Datenbank wurde wiederhergestellt. Probieren Sie nun die " + "Verbindung zur Datenbank '" + newDbName + "' zu testen.");
									}
								});
							} catch (SQLException e) {
								ExceptionUtil.log(e, "Konnte Datenbankdatei nicht anhängen " + fileName);
	...
	public void propertyChange(PropertyChangeEvent event) {
		...
		if (event.getSource().equals(toDirField)) {
			if (toDirField.getStringValue() != null && toDirField.getStringValue().length() > 0) {
				...
				if (showDirWarning && !(url.indexOf("localhost") > -1 || url.indexOf("127.0.0.1") > -1)) {
		...
		if (event.getSource().equals(this.gstoolDumpFile)) {
			if (gstoolDumpFile.getStringValue() != null && gstoolDumpFile.getStringValue().length() > 0) {
				...
				if (showWarning && !(url.indexOf("localhost") > -1 || url.indexOf("127.0.0.1") > -1)) {
					showWarning = false;
