...
public class GSImportAttachPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	public void createFieldEditors() {
		...
		gstoolDumpFile = new FileFieldEditor(PreferenceConstants.GSTOOL_FILE, "GSTOOL-Datei anhängen", getFieldEditorParent()) {
			@Override
		...
		attachDb = new StringFieldEditor(PreferenceConstants.GS_DB_ATTACHDB, "Datenbank anhängen als Name", getFieldEditorParent());
		addField(attachDb);
	...
	private void createAttachButton() {
		...
		button.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (!found) {
					Logger.getLogger(this.getClass()).debug("No database defined.");
					return;
				}
				...
				boolean doIt = MessageDialog.openConfirm(getShell(), "Datenbank anhängen", "Verbindung wird hergestellt mit " + urlString + "\n\nDie Datei '" + fileName + "' wird angehängt als Datenbank '" + newDbName + "'.\nDiese Datenbank sollte nicht existieren, alle evtl. existierenden Daten werden überschrieben." + "\n\nWollen Sie fortfahren?");
				if (!doIt) {
					return;
				}
						...
						public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
							...
							try {
								...
								Display.getDefault().syncExec(new Runnable() {
									public void run() {
										MessageDialog.openInformation(getShell(), "Hurra", "Die Datenbank wurde angehängt. Probieren Sie nun die " + "Verbindung zur Datenbank '" + newDbName + "' zu testen.");
									}
								});
