...
public class ImportCSVWizard extends Wizard {
        ...
        if (page == entityPage) {
            try {
                ...
                if (firstLine == null || firstLine.length == 0) {
                    final String message = NLS.bind(Messages.ImportCSVWizard_2,
                            entityPage.getCSVDatei().getPath());
                    LOG.warn(message);
    ...
    /**
     * Creates {@link SyncRequest} instance out of the values and settings given
     * in <code>propertyPage</code>.
     * 
     * @return
     * @throws IOException
     */
        ...
        for (List<String> tableLine : propertyPage.getContent()) {
            ...
            for (String tableCell : tableLine) {
                if (j == 0) {
                    ...
                    // With this approach the import will be fine but the CSV
                    // format
                    // will not have the ability to properly represent
                    // verinice's
                    // way of handling multiple property values (which are lists
                    // internally).
