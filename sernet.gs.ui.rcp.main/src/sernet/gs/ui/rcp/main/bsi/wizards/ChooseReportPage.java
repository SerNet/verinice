...
/**
 * WizardPage to choose the kind of report that should be generated from the
 * List of <code>IBSIReports</code>.
 * 
...
public class ChooseReportPage extends WizardPage {
    ...
    private static final Logger LOG = Logger.getLogger(ChooseReportPage.class);
    ...
    protected ChooseReportPage() {
        super("Report auswählen"); //$NON-NLS-1$
        setTitle(Messages.ChooseReportPage_1);
        setDescription(Messages.ChooseReportPage_2);
    }
    ...
    public void createControl(Composite parent) {
        ...
        reportsTable = new Table(container, SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER);
        ...
        reportsTable.addSelectionListener(new SelectionListener() {
            ...
            public void widgetSelected(SelectionEvent e) {
                if (reportsTable.getSelectionCount() > 0) {
                    ...
                    getExportWizard().setReport((IBSIReport) reportsTable.getSelection()[0].getData());
                }
            }
        });

        nameColumn = new TableColumn(reportsTable, SWT.LEFT);
        nameColumn.setText(Messages.ChooseReportPage_3);
    ...
    private void initContents() {
		...
		report = new ISMTypedItemReport(reportProperties);
		item = new TableItem(reportsTable, SWT.NULL);
		item.setText(0, report.getTitle());
		item.setData(report);
    ...
    private Properties loadReportProperties() {
        try {
            ...
            File config = new File(CnAWorkspace.getInstance().getConfDir() + File.separator + IBSIReport.PROPERTY_FILE);
            ...
            if (is != null) {
                ...
                LOG.error("Konnte Report Default-Felder nicht laden."); //$NON-NLS-1$
            }
        } catch (IOException e) {
            LOG.error("Konnte Report Default-Felder nicht laden.", e); //$NON-NLS-1$
        }
        return null;
    }
