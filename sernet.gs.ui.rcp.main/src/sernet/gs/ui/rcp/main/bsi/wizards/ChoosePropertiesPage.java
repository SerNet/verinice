...
/**
 * Dialog that lets the user choose which properties from all given HUI Entities
 * should be included in a report output.
 * 
 * 
 * @author koderman[at]sernet[dot]de
 * 
 * @see Hitro-UI framework documentation for a description of Entity and
 *      Property classes
 */
public class ChoosePropertiesPage extends WizardPage {
    ...
    protected ChoosePropertiesPage() {
        super("Spalten auswählen"); //$NON-NLS-1$
        setTitle(Messages.ChoosePropertiesPage_1);
        setDescription(Messages.ChoosePropertiesPage_2);
    }
    ...
    private void updatePropertiesToExport() {
        ...
        for (EntityType type : shownEntityTypes) {
            for (IEntityElement child : type.getElements()) {
                if (child instanceof PropertyType && viewer.getChecked(child)) {
                    getExportWizard().addShownProperty(type.getId(), child.getId());
                }

                if (child instanceof PropertyGroup) {
                    for (PropertyType propType : ((PropertyGroup) child).getPropertyTypes()) {
                        if (viewer.getChecked(propType)) {
                            getExportWizard().addShownProperty(type.getId(), propType.getId());
                        }
                    }
                }
            }
    ...
    private void initBsiItems() {
        ...
        try {
            ...
            for (CnATreeElement item : items) {
                ...
                EntityType entityType = HitroUtil.getInstance().getTypeFactory().getEntityType(item.getEntity().getEntityType());
            ...
            ExceptionUtil.log(e, Messages.ChoosePropertiesPage_3);
        }
    }
