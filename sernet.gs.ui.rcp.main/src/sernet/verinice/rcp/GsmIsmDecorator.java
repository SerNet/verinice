...
/**
 * Creates an overlay Decorator (blue, yellow or red dot) for Vulnerabilities
 * and Scenarios based on values coming from the Greenbone OMM API (see
...
public class GsmIsmDecorator extends LabelProvider implements ILightweightLabelDecorator {
    ...
    public void decorate(Object element, IDecoration decoration) {
        ...
        if (element instanceof CnATreeElement && prefEnabled()
                && isApplicable((CnATreeElement) element)) {
    ...
    private boolean isApplicable(CnATreeElement treeElement) {
        return IncidentScenario.TYPE_ID.equals(treeElement.getTypeId())
                || Vulnerability.TYPE_ID.equals(treeElement.getTypeId());
    }
