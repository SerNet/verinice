...
/**
 * Creates an overlay Decorator (green, yellow or red dot) for Assets,
 * Scenarios, Threats and Vulnerabilities based on different risk analysis
 * values.
 * 
...
public class RiskAnalysisDecorator extends LabelProvider implements ILightweightLabelDecorator {
            ...
            if (prefEnabled() && isApplicable(treeElement)) {
                ...
                if (riskLevel != NO_RISK_VALUE) {
                    decoration.addOverlay(
                            ImageCache.getInstance().getImageDescriptor(getImagePath(riskLevel)));
                }
            }
        }
    }
    ...
    private int getRiskLevelForIncidentScenario(CnATreeElement element)
    ...
    private String getImagePath(int riskLevel) {
        switch (riskLevel) {
        case COMBINED_RISK_VALUE_HIGH:
            return IMAGE_PATH_RED;
        case COMBINED_RISK_VALUE_MEDIUM:
            return IMAGE_PATH_YELLOW;
        case COMBINED_RISK_VALUE_LOW:
