...
public class CnAValidationView extends RightsEnabledView implements ILinkedWithEditorView {
		...
		doubleClickAction = new Action() {
			...
			public void run() {
				if (viewer.getSelection() instanceof IStructuredSelection
						&& ((IStructuredSelection) viewer.getSelection()).getFirstElement() instanceof CnAValidation) {
					try {
						CnAValidation validation = (CnAValidation) ((IStructuredSelection) viewer.getSelection())
								.getFirstElement();
	...
	private static class TableSorter extends ViewerSorter {
		...
		private int compareByHint(CnAValidation a1, CnAValidation a2) {
			...
			return hintA1.compareToIgnoreCase(hintA2);
		}
		...
		private int compareByType(CnAValidation a1, CnAValidation a2) {
			...
			return typeA1.compareToIgnoreCase(typeA2);
		}
		...
		if (currentCnaElement != null) {
			...
			if (isLinkingActive()) {
				...
				if ((selectedValidation != null
						&& !selectedValidation.getElmtDbId().equals(this.currentCnaElement.getDbId()))
