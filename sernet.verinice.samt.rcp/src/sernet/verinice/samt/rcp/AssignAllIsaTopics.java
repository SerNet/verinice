...
public class AssignAllIsaTopics implements IObjectActionDelegate, RightEnabledUserInteraction {
    ...
    public void run(IAction action) {
        ...
        CnATreeElementSelectionDialog dialog = new CnATreeElementSelectionDialog(window.getShell(),
                PersonIso.TYPE_ID, firstSelected);
        ...
        try {
            ...
            InfoDialogWithShowToggle.openInformation(Messages.AssignAllIsaTopics_0,
                    Messages.bind(Messages.AssignAllIsaTopics_1, numberOfTopics),
                    Messages.AssignAllIsaTopics_2, SamtPreferencePage.INFO_CONTROLS_LINKED);
            ...
            for (CnATreeElement child : children) {
                if (child != null) {
                    if (ControlGroup.TYPE_ID.equals(child.getTypeId())) {
                        linkChildren(child, personList);
                    }
                    if (child instanceof IControl) {
                        CnAElementHome.getInstance().createLinksAccordingToBusinessLogicAsync(child,
                                personList, SamtTopic.REL_SAMTTOPIC_PERSON_ISO);
                        numberOfTopics++;
                    }
                }
            }
    ...
    public void selectionChanged(IAction action, ISelection selection) {
        ...
        if (selection instanceof IStructuredSelection) {
            ...
            Iterator iterator = ((IStructuredSelection) selection).iterator(); iterator
                    .hasNext();) {
